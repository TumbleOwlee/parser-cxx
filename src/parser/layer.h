 ...
 */
class Layer : public std::variant<std::monostate, ::parse::Value,
                                  ::parse::compare::Comparison> {
public:
  /*!
  ...
  template <typename T>
  Layer(T value)
      : std::variant<std::monostate, ::parse::Value,
                     ::parse::compare::Comparison>(value) {}
};
...
auto operator<<(
    std::ostream &out,
    std::expected<std::vector<parse::Layer>, parse::Error> const &result)
    -> std::ostream &;
